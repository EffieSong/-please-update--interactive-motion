...
function changeWithClickObject(startValue) {
    ...
    this.addWhenPressed = function (inc) {
        if (mouseIsPressed && !this.preMouseState) {
            let item = this;
            setTimeout(() => {
                item.reset(item.value, item.value + inc);
            }, 300);
        }
...
let progress = 0; //0-1
let speed = 0.1;
let button = document.querySelector('.qiugengxin');

button.onclick = function () {
    console.log(explosiveEmojis.length);
    if (explosiveEmojis.length == 1 || explosiveEmojis[explosiveEmojis.length - 1].exploded) 
    profileJump();
};
...
function profileJump() {
    let start = 100;
    let end = 130;
    let y = start + (start - end) * progress;
    let profile = document.querySelector('.profile');
    profile.style.top = y + 'px';
    if (progress > 0.5) speed *= (-1);
    progress += speed;
    if (progress > 0) requestAnimationFrame(profileJump);
    if (progress <= 0) {
        progress = 0;
        speed = 0.11;
    }
